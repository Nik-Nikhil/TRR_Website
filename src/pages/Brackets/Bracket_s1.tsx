...
const ELBOW = 14      // tiny elbow length
...
function Header({
  ...
  return (
    <div
      className="mb-6 py-3 text-center font-extrabold rounded-md text-sm tracking-wide"
      style={{
        ...
        {data.upperBracket.rounds.map((r, ri) => {
          ...
          return (
            <div key={r.id} style={{ marginLeft: ri ? COL_GAP : 0 }}>
              <p className="text-xs text-center mb-4">{r.label}</p>
              <div
                className="relative"
                style={{
                  width: CARD_W,
