// src/GroupStage/GroupStageTable.tsx
...
export default function GroupStageTable({ data, season }: { data: GroupStageData; season: string }) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-[1400px] px-4 flex flex-col items-center gap-4">
        ...
        <div className="w-full flex items-center justify-between">
          ...
          <div className="text-center">
            ...
            >
              ...
              {/* Exact replica of the i button in the table */}
              <div
...
function LiquipediaStyleGroup({
  ...
  const matchMatrix = useMemo(() => {
    ...
    Object.values(series).forEach((seriesMatches) => {
      seriesMatches.forEach((match) => {
        // Initialize both teams if not exists
        if (!matrix[match.team1]) matrix[match.team1] = {}
        if (!matrix[match.team2]) matrix[match.team2] = {}
        
        // Store the result from team1's perspective
        matrix[match.team1][match.team2] = match.result
        
        // Store the reversed result from team2's perspective
  ...
  const getRankColor = (rank: number) => {
    if (rank === 1) return "#22c55e" // green - 1st place
    if (rank <= 3) return "#eab308" // yellow - 2nd-3rd place
    return "#6b7280" // gray - eliminated
  }

  const isQualified = (rank: number) => rank <= 3
      ...
      <div className="overflow-x-hidden">
        <table style={{ borderCollapse: "collapse", tableLayout: "fixed", margin: "0 auto" }}>
          <colgroup>
            ...
            {teams.map((_, i) => <col key={i} style={{ width: "50px" }} />)}
            <col style={{ width: "35px" }} />
            <col style={{ width: "35px" }} />
            <col style={{ width: "35px" }} />
              ...
              {teams.map((t) => (
                <th key={t.captain} className="liqui-th" style={{ textAlign: "center", padding: "6px 2px" }}>
                  <div className="flex flex-col items-center gap-0.5">
                    ...
                    />
                    <span className="text-[8px] font-mono font-bold">{t.captain}</span>
                  </div>
                </th>
              ))}
            ...
            {teams.map((team, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Team */}
                  <td className="liqui-td" style={{ padding: "6px 8px" }}>
                    <div className="flex items-center gap-1.5">
                      <div 
                        className="w-2 h-2 rounded-full flex-shrink-0"
                        style={{ background: team.color, boxShadow: `0 0 6px ${team.color}` }}
                      />
                      <span className="text-[11px] font-bold" style={{ color: "#f0ede6" }}>
                        {team.captain}
                      </span>
                      {qualified && (
                        <div 
                          ...
                          title={rank === 1 ? "Qualified to Semi-Finals" : "Qualified to Quarter-Finals"}
                        >
                          Q
                        </div>
                      )}
